...
void DelayLineModule::setDelayForDelayLine(float delayInSamples, float velocity, float frequency) {
    ...
    //lowpass
    float brightness = juce::jmap(velocity, 0.0f, 1.0f, 4.0f, 25.0f);
    float cutoffOffset = (delayInSamples > 200.0f) ? 300.0f : 100.0f;
    float cutoff = cutoffOffset + (frequency * brightness);

    float lowPassRes = 0.707f;
    lowpassFilter.setCutoffFrequency(juce::jlimit(150.0f, 6000.0f, cutoff));
    lowpassFilter.setResonance(lowPassRes);

    //allpass
    float stiffness = frequency * 250.0f + (frequency * frequency * 0.15f);
    stiffness = juce::jlimit(2000.0f, 18000.0f, stiffness);
